...
function createAllocator({ pool, id, notifier }) {
  ...
  const appointmentTime = value => { try { return new Intl.DateTimeFormat('en-KE',{dateStyle:'medium',timeStyle:'short',timeZone:'Africa/Nairobi'}).format(new Date(value)); } catch (_) { return ''; } };
  ...
  async function offer(order, candidate) {
    ...
    notifier.partner({
      ...
      subject:'New Lune booking request',
      html:`<p>A Lune booking is waiting for ${html(candidate.location_name)}.</p><p><strong>${html(order.item_snapshot?.title || 'Lune appointment')}</strong><br>${html(appointmentTime(order.scheduled_for))}<br>${html(order.customer_name || 'Client')} · KSh ${Number(order.amount_kes || 0).toLocaleString('en-KE')}</p><p>Respond within ${RESPONSE_MINUTES} minutes: <a href="${link}">Open the request</a></p>`
    }).catch(() => {});
  ...
  async function allocateOrder(orderId) {
    ...
    const offered = await offer(order,candidate);
  ...
  async function acceptAllocation(allocationId, actor = {kind:'partner',id:null}) {
    ...
    notifier.customer({
      ...
      subject:'Your Lune appointment is ready',
      html:`<p>Hi ${order.customer_name || 'there'}, ${allocation.location_name} is ready for you.</p><p>Complete payment to make the time yours.</p>`
    }).catch(() => {});
    return order;
  }
