(() => {
  ...
  function actions(){
    ...
    if(['completed','reviewed'].includes(order.status)){out.push('<button class="ops-button lacquer" type="button" data-reorder>Do this again <span aria-hidden="true">→</span></button>');out.push('<button class="ops-button quiet" type="button" data-evolve>Same direction, something new <span aria-hidden="true">→</span></button>');}
    ...
    target.querySelector('[data-evolve]')?.addEventListener('click',()=>{const url=new URL('evolve.html',location.href);url.searchParams.set('order',order.id);const t=storedToken();if(t)url.searchParams.set('token',t);location.href=url.toString();});
  }
  ...
  function review(){
    ...
    if(order.review){target.innerHTML=`<section class="ops-card" style="margin-top:22px"><div class="eyebrow">KEPT WITH LUNE</div><h3>Thank you for letting us see it.</h3><p class="ops-muted">${esc(order.review.comment||'Your feedback is with Lune.')}</p>${order.review.photo_data_url?`<img src="${esc(order.review.photo_data_url)}" alt="Your finished Lune set" style="display:block;width:min(280px,100%);margin-top:16px;border-radius:12px">`:''}</section>`;return;}
    target.innerHTML=`<section class="ops-card" style="margin-top:22px"><div class="eyebrow">HOW DID IT TURN OUT?</div><h3>We are excited to see the finished set.</h3><p class="ops-muted">Tell us how it felt — and if you want, leave a photo of the result. It helps Lune keep the kind of work you came for close.</p><div class="rating-row" data-rating>${[1,2,3,4,5].map(n=>`<button type="button" data-rate="${n}">${n}</button>`).join('')}</div><div class="ops-field" style="margin-top:14px"><textarea data-review-copy placeholder="What did you love? What should feel better next time?"></textarea></div><div class="ops-field"><label class="ops-label">A photo of your finished set <input data-review-photo type="file" accept="image/png,image/jpeg,image/webp"></label></div><button class="ops-button" type="button" data-review-submit>Keep this with Lune <span aria-hidden="true">→</span></button><p class="ops-status" data-review-status></p></section>`;
    ...
    target.querySelector('[data-review-submit]')?.addEventListener('click',async()=>{const node=target.querySelector('[data-review-status]');if(!rating){node.textContent='Choose a rating first.';return;}try{const file=target.querySelector('[data-review-photo]')?.files?.[0];const photoDataUrl=file?await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=reject;reader.readAsDataURL(file)}):null;await request(`/orders/${encodeURIComponent(order.id)}/review`,{method:'POST',body:{visitorId:visitor(),token:storedToken(),rating,comment:target.querySelector('[data-review-copy]').value,photoDataUrl}});await load();}catch(_){node.textContent='Could not save feedback. Try a smaller image or try again.';node.classList.add('is-error');}});
  }
  ...
  function render(){
    ...
    const discount=Number(order.discountKes||0),subtotal=Number(order.subtotalKes||order.amountKes||0);
    $('[data-order-summary]').innerHTML=`<div class="summary-row"><span>When</span><strong>${esc(when)}</strong></div><div class="summary-row"><span>Experience</span><strong>${esc(({closest:'Closest fit',select:'Lune Select',calm:'Calm',same:'Same place'})[order.experiencePreference]||order.experiencePreference||'Lune match')}</strong></div>${discount?`<div class="summary-row"><span>Before Lune offer</span><strong>${money(subtotal)}</strong></div><div class="summary-row"><span>Lune offer applied</span><strong>− ${money(discount)}</strong></div>`:''}<div class="summary-row"><span>${discount?'Total after offer':'Amount'}</span><strong>${money(order.amountKes)}</strong></div><div class="summary-row"><span>Payment</span><strong>${esc(String(order.paymentStatus||'unpaid').replace(/_/g,' '))}</strong></div>`;
